...
extern MY_HIGHP_OR_MEDIUMP vec2 holo2;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
	...
	float res = (.5 + .5* cos( (holo2.x) * 2.612 + ( field + -.5 ) *3.14));
    ...
    tex =(1.-delta)*tex + delta*RGB(hsl)*vec4(1.2,0.6,0.4,tex.a);
